...
class Admin extends Component {
    constructor(props) {
        super(props);
        this.state = { admins: [], modal: false, loader: false };
    ...
    async getAdmins() {
        ...
        const response = await fetchJSON('/admin', options);
        if (response.code === 0) {
    ...
    async toggleAdminStatus(username, status) {
        ...
        await fetchJSON(`/admin/${status ? 'disable' : 'enable'}`, options);
        this.toggleLoader();
        this.getAdmins();
    }
    render() {
        const { admins, loader } = this.state;
        return (
            <React.Fragment>
